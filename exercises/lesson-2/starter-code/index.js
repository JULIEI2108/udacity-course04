...
exports.handler = async (event) => {
  ...
  let nextKey
  try {
    limit = paraseLimit(event) || 20
    nextKey = paraseNextKey(event)
  } catch (e) {
    ...
    return {
      ...
      body: JSON.stringify({
        message: 'limit and nextKey is required'
      }),
    }
  }
...
function paraseLimit(event) {
  const limitStr = getQueryParameter(event, 'limit')
  if (!limitStr) {
    return undefined
  }
  const limt = parseInt(limitStr)
  if (limit <= 0) {
